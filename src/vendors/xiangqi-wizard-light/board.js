...
Board.prototype.postAddMove = function(mv, computerMove) {
  ...
  if (vlRep > 0) {
    ...
    if (vlRep > -WIN_VALUE && vlRep < WIN_VALUE) {
      ...
      alertDelay("雙方不變作和，辛苦了！");
    } else if (computerMove == (vlRep < 0)) {
      this.playSound("loss");
      this.result = RESULT_LOSS;
      alertDelay("長打作負，請不要氣餒！");
    } else {
      this.playSound("win");
      this.result = RESULT_WIN;
      alertDelay("長打作負，祝賀你取得勝利！");
    }
    this.postAddMove2();
    this.busy = false;
    return;
...
Board.prototype.postMate = function(computerMove) {
  alertDelay(computerMove ? "請再接再厲！" : "祝賀你取得勝利！");
  this.postAddMove2();
  this.busy = false;
}
